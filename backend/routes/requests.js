...
router.post('/', (req, res) => {
  ...
  const faculty = db.prepare('SELECT id FROM faculty WHERE id = ? AND approved = 1 AND is_available = 1').get(faculty_id);
  if (!faculty) return res.status(400).json({ error: 'Faculty not found or not available' });

  const student = db.prepare('SELECT max_teams FROM students WHERE id = ?').get(req.user.id);
  const maxGuides = student ? student.max_teams : 1;
  const { cnt: acceptedCount } = db.prepare(
    "SELECT COUNT(*) AS cnt FROM requests WHERE student_id = ? AND status = 'accepted'"
  ).get(req.user.id);
  if (acceptedCount >= maxGuides) {
    return res.status(400).json({ error: `You have reached your guide limit (${maxGuides})` });
  }

  const { problem_statement } = req.body;
  if (!problem_statement || !problem_statement.trim()) {
    return res.status(400).json({ error: 'problem_statement is required' });
  }

  try {
    db.prepare(
      'INSERT OR REPLACE INTO requests (student_id, faculty_id, status, problem_statement) VALUES (?, ?, ?, ?)'
    ).run(req.user.id, faculty_id, 'pending', problem_statement.trim());
...
router.get('/incoming', (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT r.id, r.student_id, r.status, r.created_at, r.problem_statement,
...
router.patch('/:id/status', (req, res) => {
  ...
  if (status === 'accepted') {
    const studentRow = db.prepare('SELECT max_teams FROM students WHERE id = ?').get(request.student_id);
    const studentMax = studentRow ? studentRow.max_teams : 1;
    const { cnt: studentAccepted } = db.prepare(
      "SELECT COUNT(*) AS cnt FROM requests WHERE student_id = ? AND status = 'accepted'"
    ).get(request.student_id);

    if (studentAccepted >= studentMax) {
      const otherPending = db.prepare(
        "SELECT r.id, f.name AS faculty_name FROM requests r JOIN faculty f ON f.id = r.faculty_id WHERE r.student_id = ? AND r.status = 'pending' AND r.id != ?"
      ...
      for (const other of otherPending) {
        ...
        db.prepare('INSERT INTO student_notifications (student_id, message) VALUES (?, ?)').run(
          request.student_id,
          `Your request to Prof. ${other.faculty_name} was automatically rejected since you have reached your guide limit.`
        );
      }
    }
  }
