...
router.get('/students', (req, res) => {
  const rows = db.prepare(
    'SELECT id, name, email, student_id, max_teams, created_at FROM students ORDER BY created_at DESC'
  ).all();
  res.json(rows);
});

// Bulk update — must be before /:id route to avoid "bulk" being matched as an id
router.patch('/students/bulk/max-guides', (req, res) => {
  const { max_guides, student_id_prefix } = req.body;
  if (typeof max_guides !== 'number' || max_guides < 1 || max_guides > 10) {
    return res.status(400).json({ error: 'max_guides must be 1–10' });
  }
  let result;
  if (student_id_prefix && student_id_prefix.trim().length > 0) {
    result = db.prepare(
      'UPDATE students SET max_teams = ? WHERE student_id LIKE ?'
    ).run(max_guides, `${student_id_prefix.trim()}%`);
  } else {
    result = db.prepare('UPDATE students SET max_teams = ?').run(max_guides);
  }
  res.json({ message: `Updated ${result.changes} student(s)` });
});

router.patch('/students/:id/max-guides', (req, res) => {
  const { max_guides } = req.body;
  if (typeof max_guides !== 'number' || max_guides < 1 || max_guides > 10) {
    return res.status(400).json({ error: 'max_guides must be 1–10' });
  }
  const result = db.prepare('UPDATE students SET max_teams = ? WHERE id = ?').run(max_guides, req.params.id);
