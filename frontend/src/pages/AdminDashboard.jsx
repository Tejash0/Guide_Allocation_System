...
import { getStats, getStudents, getFaculty, approveFaculty, removeFaculty, assignSlots, setStudentMaxGuides, bulkSetMaxGuides } from '../api/admin.js';
...
export default function AdminDashboard() {
  ...
  const [bulkMaxGuides, setBulkMaxGuides] = useState(1);
  const [bulkPrefix, setBulkPrefix] = useState('');
  const [bulkMsg, setBulkMsg] = useState('');
  const [guideSaving, setGuideSaving] = useState(null);
      ...
      <div style={{ display: 'flex', height: '100vh', background: '#ede9e3', fontFamily: "'Trebuchet MS', 'Gill Sans MT', sans-serif" }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '28px' }}>
            ...
            {activeTab === 'Students' && (
              <div className="anim" style={{ animationDelay: '0s' }}>
                {/* Bulk max-guides control */}
                <div style={{ background: '#fff', borderRadius: 12, padding: '14px 18px', marginBottom: 14, boxShadow: '0 1px 2px rgba(0,0,0,0.05)', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ color: '#6b7280', fontSize: '0.8rem', fontWeight: 600 }}>Set max guides:</span>
                  <input
                    type="number" min={1} max={10} value={bulkMaxGuides}
                    onChange={e => setBulkMaxGuides(Number(e.target.value))}
                    style={{ width: 56, padding: '4px 8px', border: '1px solid #d0d9e4', borderRadius: 6, fontSize: '0.82rem', fontFamily: 'inherit' }}
                  />
                  <input
                    placeholder="Roll prefix (e.g. 21BCE) — blank = all students"
                    value={bulkPrefix}
                    onChange={e => setBulkPrefix(e.target.value)}
                    style={{ flex: 1, minWidth: 200, padding: '4px 10px', border: '1px solid #d0d9e4', borderRadius: 6, fontSize: '0.82rem', fontFamily: 'inherit' }}
                  />
                  <button
                    onClick={async () => {
                      const r = await bulkSetMaxGuides(bulkMaxGuides, bulkPrefix);
                      setBulkMsg(r.message || r.error || 'Done');
                      setStudents(await getStudents());
                      setTimeout(() => setBulkMsg(''), 3000);
                    }}
                    style={{ padding: '5px 14px', background: '#1a0a0a', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 600, cursor: 'pointer', fontSize: '0.8rem', fontFamily: 'inherit' }}
                  >Apply</button>
                  {bulkMsg && <span style={{ color: '#166534', fontSize: '0.78rem', fontWeight: 600 }}>{bulkMsg}</span>}
                </div>

                <TableCard title={`All Students (${students.length})`} loading={loading}>
                  {students.length === 0 ? (
                    <EmptyRow colspan={5} message="No students registered yet" />
                  ) : (
                    <table className="a-table">
                      <thead>
                        <tr><th>Name</th><th>Email</th><th>Student ID</th><th>Max Guides</th><th>Registered</th></tr>
                      </thead>
                      <tbody>
                        {students.map(s => (
                          <tr key={s.id}>
                            ...
                            <td>
                              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                                <input
                                  ...
                                  defaultValue={s.max_teams ?? 1}
                                  key={s.id + '-' + s.max_teams}
                                  onBlur={async (e) => {
                                    const val = Number(e.target.value);
                                    if (val < 1 || val > 10) return;
                                    setGuideSaving(s.id);
                                    await setStudentMaxGuides(s.id, val);
                                    setGuideSaving(null);
                                  }}
                                  style={{ width: 50, padding: '2px 6px', border: '1px solid #d0d9e4', borderRadius: 4, fontSize: '0.8rem', fontFamily: 'inherit', opacity: guideSaving === s.id ? 0.5 : 1 }}
                                />
                              </div>
                            </td>
